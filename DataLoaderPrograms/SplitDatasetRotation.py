...
def split_and_process_dataset():
    ...
    test_image_counter = 1  # Starting number for test images
    train_image_counter = 1  # Starting number for train images
    test_input_count = 0  # Count of input images for reporting
    train_input_count = 0  # Count of input images for reporting
    ...
    for subdir in test_subdirs:
        ...
        for img in images:
            ...
            test_input_count += 1
            test_image_counter = process_and_recombine_image(
                src_path, test_dir, test_image_counter, target_combined_size
            )
    ...
    for subdir in train_subdirs:
        ...
        for img in images:
            src_path = os.path.join(source_subdir, img)
            train_input_count += 1
            train_image_counter = process_and_recombine_image(
                src_path, train_dir, train_image_counter, target_combined_size
            )
    
    print(f"Processed {len(subdirs)} total folder groups:")
    print(f"  {len(train_subdirs)} folder groups ({train_input_count} input images, {train_image_counter-1} output images) moved to train")
    print(f"  {len(test_subdirs)} folder groups ({test_input_count} input images, {test_image_counter-1} output images) moved to test")

def process_and_recombine_image(src_path, dest_dir, image_counter, target_combined_size):
    """Split, crop, resize, rotate both halves by the same angle, and recombine a 3840x1080 image."""
    # Open the image
    ...
    # Rotate both foggy and clear images by the same angle
    for angle in angles:
        # Rotate foggy and clear images
        foggy_rotated = foggy_resized if angle == 0 else foggy_resized.rotate(angle, expand=True)
        clear_rotated = clear_resized if angle == 0 else clear_resized.rotate(angle, expand=True)

        # Ensure rotated images are the correct size (in case rotation changes dimensions)
        foggy_rotated = foggy_rotated.resize((half_width, half_height), Image.Resampling.LANCZOS)
        clear_rotated = clear_rotated.resize((half_width, half_height), Image.Resampling.LANCZOS)

        # Recombine into a single image
        combined = Image.new("RGB", (combined_width, half_height))
        combined.paste(foggy_rotated, (0, 0))           # Foggy on left
        combined.paste(clear_rotated, (half_width, 0))  # Non-foggy on right

        # Save the combined image with sequential numbering
        dest_path = os.path.join(dest_dir, f"{image_counter}.png")
        combined.save(dest_path, "PNG")
        image_counter += 1  # Increment counter for the next image

    return image_counter  # Return updated counter for the next call
